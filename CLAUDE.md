...
- `POST /api/canopy-sessions/` — Create a session (`agent_slug`, `title`, `metadata`); tenant route `/api/w/{ws}/canopy-sessions/` creates in that workspace (cross-workspace new-chat).
- `GET /api/canopy-sessions/` — List my sessions (creator-scoped; flat mount = across all my workspaces).
- `GET /api/canopy-sessions/{id}` — Session + transcript. `POST /api/canopy-sessions/{id}/send` — Send a message (`text`, `client_id`).
