...
- **Design tokens are the single source of truth (no raw palette literals).** The whole authenticated app styles off semantic tokens defined in `frontend/src/index.css` (`@theme inline` + `:root`) — a "Warm Earth" dark theme tuned to match `stone-*`/`orange-400`. Do **not** introduce raw Tailwind palette literals (`stone-*`, `orange-*`, `zinc-*`, `slate-*`, `red-*`, `amber-*`, `emerald-*`, `sky-*`, `violet-*`); use the tokens:
