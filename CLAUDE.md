...
- `GET /api/agents/needs-you` — Fleet-wide supervisor inbox: every agent's needs-you in one call, ranked busiest-first, with a `total_waiting` count. One round trip instead of an N+1 fan-out (which is what the menubar panel does). Drives `/supervisor`
...
**Phases 0-2 shipped.** `needs_you` reads Items **alongside** its remaining projections, and `apps/push` watches `Item` — additive on purpose: switching to items-only before tasks (Phase 4) and run gates (Phase 3) migrate would empty the inbox, not unify it. Each projection deletes itself as its producer moves; `needs_you` becomes a pure query after Phase 4. Still projections: tasks, run gates, #218's schedule nag, and syncs/work-products (the `notify` band — headed for `/timeline`, its own decision). `gate="product_findings"` + the runner's `reviews.py` stay until Ada's repo posts Items instead — retiring them first would strand her next audit with no decision surface.
...
- **Push fires on an increase, never a decrease**: `apps.agents.services.needs_you()` is an aggregation (tasks + run gates + failed steps), not a single event, so nothing naturally emits "the fleet needs you now." `AgentWaitingSnapshot` (`apps/push`) holds the last `waiting_count` per agent; `post_save`/`post_delete` signals mark an agent dirty and `transaction.on_commit` coalesces everything dirtied in one transaction into a single recompute per agent — so a bulk `tasks/sync` of N rows still sends at most one push. A drop in count updates the snapshot but never sends. **Known gap:** an agent listed in `AGENT_RUNS_DRIVE_ROOTS` (Drive-backed run store) has no DB rows for its run gates, so no signal fires and its gate-opens don't push — only task pushes work for such an agent. The map is `{}` today, so nothing is affected yet. See `apps/push/signals.py`.
