"""SP2a Task 5 — the /api/chat surface: create, get, send (stub inline), tenancy."""
...
def test_create_and_get_empty_session(client):
    r = client.post("/api/chat/", data={"agent_slug": "echo", "title": "T"}, content_type="application/json")
    assert r.status_code == 200, r.content
    body = r.json()
    assert body["agent_slug"] == "echo"
    sid = body["id"]

    detail = client.get(f"/api/chat/{sid}")
    assert detail.status_code == 200
    assert detail.json()["messages"] == []


def test_send_runs_stub_and_transcript_appears(client):
    sid = client.post("/api/chat/", data={"agent_slug": "echo"}, content_type="application/json").json()["id"]
    r = client.post(f"/api/chat/{sid}/send", data={"text": "hi"}, content_type="application/json")
    assert r.status_code == 200, r.content
    assert r.json()["message"]["role"] == "user"

    detail = client.get(f"/api/chat/{sid}").json()
    roles = [m["role"] for m in detail["messages"]]
    assert roles == ["user", "assistant"]  # the stub executed and projected


def test_empty_text_rejected(client):
    sid = client.post("/api/chat/", data={}, content_type="application/json").json()["id"]
    r = client.post(f"/api/chat/{sid}/send", data={"text": "   "}, content_type="application/json")
