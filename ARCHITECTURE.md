...
| `chat` | **framework** | Live chat sessions — the interactive front-door to a durable `harness` Turn. A `Session` "send" enqueues a session-target Turn; the assistant stream lands in the `TurnEvent` ledger and is projected into `Message` rows. Agent-agnostic (opaque `metadata` for product linkage). Named `chat` (not `sessions`) to avoid the `django.contrib.sessions` label + `session_sharing` route collisions. Wave 4 SP2. |
